"""Build the hillshade tiles for one box out of Lantmäteriet's height model.
...
-- analysis/docs/abisko-decisions.md §6.6. This reads the same cached mosaic
...
Run with::
    ...
    command make shade                              # the Abisko box, z8 to z15
...
#: The finest level built. Past it the model has nothing more to give: the
#: heights are smoothed by one post before they are differentiated, so a z16
#: tile would draw the same ground on a finer grid (§6.6).
MAX_ZOOM = 15
...
DEFAULT_OUTPUT = Path("analysis") / "output" / "shade" / "lantmateriet" / "1"
