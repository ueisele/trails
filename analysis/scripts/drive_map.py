...
@dataclass(frozen=True)
class WaterLeg:
    """Two shore positions and the measured way round the shore between them."""

    #: Latitude and longitude, in journey order, read from the page's network.
    points: tuple[tuple[float, float], tuple[float, float]]
    #: The shortest way using Shore edges alone, measured on the decoded page.
    #: A chord may beat this figure, but cannot redefine what the shore measures.
    shore_m: float
...
class Scene:
    ...
    **A check is structural or it is a figure, and neither is a park.** The
    ...
    #: credited to FKB, UT.no and N50. Phase 10 keeps water first and chooses
...
SCENES: dict[str, Scene] = {
    "lomsdal-visten": Scene(
        ...
        figures={
            ...
            "zoom before": 9,
            "zoom after": 11,
            # Phase 10 full-suite measurement: 96 → 97.8.
            "desktop: map free with nothing asked for": 97.8,
            # Phase 10 full-suite measurement: 96 → 98.
            ...
            # Phase 10 full-suite measurement: 565 → 562.
            "px of map left above it": 562,
            "sideways: px the drawing takes": 109,
            # Phase 10 full-suite measurement: 186 → 189.
            ...
            # Phase 10 full-suite measurement: 11188 → 11202.
            "m shown by a quarter-width drag": 11202,
            ...
            # Phase 10 full-suite measurement: 30 → 27.
            "and what width it says": 27,
            "readable in the light set": 15.1,
            # Phase 10 full-suite measurement: 549 → 540.
            "what it weighs": 540,
    ...
    "abisko": Scene(
        ...
        figures={
            ...
            "readable in the light set": 15.1,
            # Phase 10 full-suite measurement: 465 → 464.
            "what it weighs": 464,
    ...
    "malingsbo-kloten": Scene(
        ...
        figures={
            ...
            "phone-2 1→2, Stay on paths on: on foot m": 175.289,
            "phone-2 1→2, Stay on paths on: water m": 0,
            "phone-2 2→3, Stay on paths off: on foot m": 49.584,
            ...
            "readable in the light set": 15.1,
            # Phase 10 full-suite measurement: 652 → 647.
            "what it weighs": 647,
            "packs kept for the tiny scope and overview": 81,
...
def goal_lengths(page: Any, water: bool) -> dict[str, Any]:
    ...
    """
    saved_way = page.evaluate("() => trailsGoal.state().way")
    page.set_viewport_size({"width": 390, "height": 844})
    ...
    elif SCENE.length_endpoints is not None:
        here, there = ({"lat": lat, "lon": lon} for lat, lon in SCENE.length_endpoints)
    else:
        if not select(page, SCENE.long_chain):
    ...
    page.evaluate("way => { trailsGoal.clear(); trailsGoal.way(way); trailsChrome.close(); trailsPlan.toggle(false); }", saved_way)
    page.wait_for_timeout(400)
    page.set_viewport_size({"width": 1400, "height": 900})
    page.wait_for_timeout(400)
    return read
...
#: The state these readings borrow. Index timings and height-request counters
#: describe work done, so restoring them would conceal the work being measured.
KAYAK_PAGE_STATE = """() => {
...
def read_water_leg(
    ...
    *,
    ...
    Args:
        ...
        paths: The ground-price preference to use while reading it
    ...
    try:
        ...
        planning_width = page.locator(".trails-profile-name").evaluate(
            "node => ({used: node.scrollWidth, available: node.clientWidth, "
            "height: node.clientHeight, header: node.closest('.trails-profile-head').clientHeight})"
        )
        ...
        got: dict[str, Any] = page.evaluate(
            ...
            const direct = trailsProfilePanel.metresBetween(a.lon, a.lat, b.lon, b.lat);
            const count = Math.max(1, Math.ceil(direct / graph.water.cellM));
            let wet = 0;
            ...
            let routing = null;
            if (measureRouting) {
              const m = window.kayakMeasure, originalPop = m.Heap.prototype.pop;
              let pops = 0, joined;
              m.Heap.prototype.pop = function () { pops++; return originalPop.call(this); };
              try { joined = m.joinedRoute(graph, a, b); }
        ...
        painted(page)
    got["planningLine"] = planning_line
    got["planningWidth"] = planning_width
    got["paths"] = paths
    got["restored"] = page.evaluate(KAYAK_PAGE_STATE) == saved
    return got


def water_leg_readings(got: dict[str, Any], name: str) -> list[Reading]:
    """Require a settled scene leg and the return of the state it borrowed."""
    legs = got["state"]["legs"]
    return [
        noted(f"{name}: Stay on paths", "on" if got["paths"] else "off"),
        Reading(f"{name}: the leg settled without a failure", len(legs) == 1 and legs[0]["settled"] and not legs[0]["failed"], True),
        Reading(f"{name}: the mode, goal way and plan are put back", got["restored"], True),
    ]
...
def a_bay_is_cut_and_a_lake_is_not(page: Any) -> Check:
    """The same prices cut the deep bay and keep the lake leg on its shore."""
...
def a_carry_uses_walking_prices(page: Any) -> Check:
    """Account for the mapped and inferred ground chosen by land price."""
    name = "a carry uses walking prices"
    ...
    return Check(
        ...
        + [
            ...
            noted("the carry chosen by walking land price, m", s["walked"]),
...
def a_kayak_launches_from_the_road(page: Any) -> Check:
    """The short launch gives Kloten's road a path-priced way into the bay."""
    name = "a kayak launches from the road"
    if SCENE.stem != "malingsbo-kloten":
        return Check(name, skipped="the phone-2 and Kloten starts belong to Malingsbo-Kloten")
    p1 = (59.895949196, 15.288015230)
    p2 = (59.897507, 15.288204)
    fixtures = (
        ("phone-2 1→2", (p1, p2)),
        ("phone-2 2→3", (p2, (59.897891, 15.288320))),
        ("Kloten northern shore", (p1, (59.898026273, 15.288010427))),
    )
    readings: list[Reading] = []
    for paths in (False, True):
        for label, points in fixtures:
            got = read_water_leg(page, points, paths=paths, measure_shore=True)
            state = got["state"]
            launch = sum(part["launch"] for part in got["paddledTrack"])
            prefix = f"{label}, Stay on paths {'on' if paths else 'off'}"
            readings.extend(water_leg_readings(got, prefix))
            if label == "phone-2 1→2":
                readings.append(
                    Reading(
                        f"{prefix}: the carry follows the road", state["tally"]["sources"].get("Topografi 50 roads", 0), state["walked"], within=0.001
                    )
                )
                readings.append(Reading(f"{prefix}: the road is routed throughout", state["straight"], 0))
            else:
                readings.append(Reading(f"{prefix}: the way uses a launch", launch > 0, True))
            readings.extend(
                [
                    noted(f"{prefix}: launch m", launch),
                    stands(f"{prefix}: on foot m", round(state["walked"], 3), within=0.001),
                    stands(f"{prefix}: water m", round(state["crossed"], 3), within=0.001),
                ]
            )
    return Check(name, readings)


def the_kayak_path_switch_changes_the_carry(page: Any) -> Check:
    """Near Kloten, the higher ground price buys a longer carry along the road."""
    name = "the kayak path switch changes the carry"
    if SCENE.stem != "malingsbo-kloten":
        return Check(name, skipped="the measured ground/path competition is beside Kloten")
    points = ((59.8967, 15.2889), (59.898026273, 15.288010427))
    off = read_water_leg(page, points, paths=False)
    on = read_water_leg(page, points, paths=True)
    readings: list[Reading] = []
    for paths, got in ((False, off), (True, on)):
        prefix = f"kayak ground/path competition, Stay on paths {'on' if paths else 'off'}"
        state = got["state"]
        readings.extend(water_leg_readings(got, prefix))
        for label, metres in (
            ("on foot", state["walked"]),
            ("straight ground", state["straight"]),
            ("road", state["tally"]["sources"].get("Topografi 50 roads", 0)),
        ):
            readings.append(stands(f"{prefix}: {label} m", round(metres, 3), within=0.001))
    a, b = off["state"], on["state"]
    readings.extend(
        [
            Reading("the higher ground price reduces the straight carry", b["straight"] < a["straight"], True),
            Reading(
                "the higher ground price follows more of the road",
                b["tally"]["sources"].get("Topografi 50 roads", 0) > a["tally"]["sources"].get("Topografi 50 roads", 0),
                True,
            ),
            Reading("keeping to the road means a longer carry", b["walked"] > a["walked"], True),
        ]
    )
    return Check(name, readings)


def the_kayak_panel_counts_the_whole_way(page: Any) -> Check:
    """The compact line counts water, and a launch never becomes a walking edge."""
    name = "the kayak panel counts the whole way"
    if SCENE.kayak_shore is None:
        return Check(name, skipped="this scene has no kayak shore leg")
    readings: list[Reading] = []
    viewport = page.viewport_size
    try:
        page.set_viewport_size({"width": 390, "height": 844})
        for paddling in (True, False):
            for paths in (False, True):
                got = read_water_leg(page, SCENE.kayak_shore.points, paddling=paddling, paths=paths)
                state = got["state"]
                whole = state["walked"] + (state["crossed"] if paddling else 0)
                words = f"{whole / 1000:.2f} km"
                if paddling:
                    words += f" · {state['crossed'] / 1000:.2f} km 🛶 · {state['walked'] / 1000:.2f} km 🚶"
                prefix = f"{'kayak' if paddling else 'walking'}, Stay on paths {'on' if paths else 'off'}"
                readings.extend(water_leg_readings(got, prefix))
                readings.append(Reading(f"{prefix}: the compact length", words in got["planningLine"], True))
                readings.append(Reading(f"{prefix}: the split is shown only in kayak", "🛶" in got["planningLine"], paddling))
                if paddling:
                    readings.append(noted(f"{prefix}: compact figure pixels", got["planningWidth"]))
                    readings.append(
                        Reading(
                            f"{prefix}: the whole split fits the phone",
                            got["planningWidth"]["available"] > 0 and got["planningWidth"]["used"] <= got["planningWidth"]["available"],
                            True,
                        )
                    )
    finally:
        ...
        painted(page)
    saved = page.evaluate("() => ({kayak: trailsPlan.kayak(), paths: trailsPlan.stayOnPaths()})")
    try:
        for paths in (False, True):
            page.evaluate("paths => { trailsPlan.kayak(false); trailsPlan.stayOnPaths(paths); }", paths)
            forbidden = page.evaluate("""() => { const g=trailsGraph,w=kayakMeasure.router(g);
                return Array.from(g.sources).every((s,e)=>g.header.sources[s].kind!=='launch'||w.cost[e]===Infinity); }""")
            readings.append(Reading(f"walking, Stay on paths {paths}: every launch is unreachable", forbidden, True))
    finally:
        page.evaluate("s => { trailsPlan.kayak(s.kayak); trailsPlan.stayOnPaths(s.paths); }", saved)
        page.wait_for_function("() => !trailsPlan.busy() && !trailsGoal.state().working", timeout=120_000)
    return Check(name, readings)


def a_kayak_rounds_land_from_open_water(page: Any) -> Check:
    """Off-network water taps take a zero-land search within the grid's allowance."""
    pairs = {
        "malingsbo-kloten": ((59.9659281691193, 15.863781710359643), (59.97490373192166, 15.850783875167386)),
        "abisko": ((68.44199171587779, 18.750492230735862), (68.44250067992853, 18.774400423102342)),
        "lomsdal-visten": ((65.18147834294567, 13.333300517592484), (65.17663041216133, 13.315444388020483)),
    }
    if SCENE.stem not in pairs:
        return Check("a kayak rounds land from open water", skipped="this scene has no measured off-network pair")
    points = pairs[SCENE.stem]
    got = read_water_leg(page, points, measure_routing=True)
    state, route = got["state"], got["routing"]
    wet = page.evaluate("points => points.every(p => !!trailsGraph.waterAt(p[1], p[0]))", points)
    return Check(
        "a kayak rounds land from open water",
        water_leg_readings(got, "off-network water pair")
        + [
            Reading("the two taps lie on water", wet, True),
            Reading("neither tap snaps to the network", all(p["node"] < 0 and p.get("edge", -1) < 0 for p in state["points"]), True),
            Reading("the direct connector crosses dry ground", got["direct"]["water"] < got["direct"]["metres"], True),
            Reading("the router chooses zero land", route["land"], 0),
            Reading("the network part is paddled", route["paddled"], True),
            Reading("the search stays within its step bound", route["pops"] <= route["stepBound"], True),
            Reading("the search and public way have the same length", route["metres"], state["crossed"] + state["walked"], within=0.001),
            # Phase 2 retains cell-based prices and the finer profile tally.
            # Each connector end gets at most one cell for a shore sliver.
            Reading("the tally differs by no more than a cell per connector end", state["walked"], route["land"], within=route["allowance"]),
            Reading("the detour is longer than the direct line", state["crossed"] > got["direct"]["metres"], True),
            Reading("the detour includes paddling", state["crossed"] > 0, True),
            noted("router land / tally land / allowance, m", [route["land"], state["walked"], route["allowance"]]),
            noted("connector lengths and cell size, m", [route["connectors"], route["cellM"]]),
            noted("search pops / step bound", [route["pops"], route["stepBound"]]),
            noted("off-network water pair, latitude and longitude", points),
            noted("off-network pair paddled / on foot, m", [state["crossed"], state["walked"]]),
        ],
    )


def a_kayak_uses_water_before_land(page: Any) -> Check:
    """A connected water pair paddles, while a point placed on land is reached."""
    name = "a kayak uses water before land"
    if SCENE.kayak_shore is None:
        return Check(name, skipped="this scene has no measured kayak shore")
    # The screenshot's path identifies the peninsula, but its taps are unknown.
    # These wet shore nodes reconstruct the two sides without requiring a carry.
    points = ((59.945871, 15.258351), (59.952097, 15.273234)) if SCENE.stem == "malingsbo-kloten" else SCENE.kayak_shore.points
    chosen = page.evaluate(
        """points => {
        const g = trailsGraph, n = g.header.nodes, distance = trailsProfilePanel.metresBetween;
        const water = Array.from({length: n}, () => []), wet = new Set(), land = new Set(), paddleNodes = new Set();
        for (let e = 0; e < g.header.edges; e++) {
          const kind = g.header.sources[g.sources[e]].kind, a = g.fromNode[e], b = g.toNode[e];
          if (kind === 'paddle') {
            paddleNodes.add(a); paddleNodes.add(b);
            water[a].push(b);
            if (!g.oneWay[e]) water[b].push(a);
            for (const node of [a, b]) if (g.waterAt(g.nodeLon[node], g.nodeLat[node])) wet.add(node);
          }
          if (kind === 'path') for (const node of [a, b])
            if (!g.waterAt(g.nodeLon[node], g.nodeLat[node])) land.add(node);
        }
        // A shore junction can belong to both a path and water. Choose a
        // land point beyond the water grid's sampling uncertainty instead.
        for (const node of land) {
          const lat = g.nodeLat[node], lon = g.nodeLon[node], dy = g.water.cellM / 111320;
          const dx = dy / Math.cos(lat * Math.PI / 180);
          let dry = !paddleNodes.has(node);
          for (const x of [-1, 0, 1]) for (const y of [-1, 0, 1])
            if (g.waterAt(lon + x * dx, lat + y * dy)) dry = false;
          if (!dry) land.delete(node);
        }
        function nearest(nodes, point) {
          let best = -1, metres = Infinity;
          for (const node of nodes) {
            const away = distance(point[1], point[0], g.nodeLon[node], g.nodeLat[node]);
            if (away < metres) { best = node; metres = away; }
          }
          if (best < 0) throw new Error('the scene has no eligible node');
          return best;
        }
        const pair = points.map(p => nearest(wet, p)), onLand = nearest(land, points[1]);
        // Reachability is checked on paddle arcs alone, independently of prices.
        const seen = new Uint8Array(n), queue = new Int32Array(n);
        let read = 0, size = 1;
        queue[0] = pair[0]; seen[pair[0]] = 1;
        while (read < size) {
          if (read >= n) throw new Error('water traversal exceeds the graph');
          const node = queue[read++];
          for (const other of water[node]) if (!seen[other]) {
            if (size >= n) throw new Error('water queue exceeds the graph');
            seen[other] = 1; queue[size++] = other;
          }
        }
        const position = node => [g.nodeLat[node], g.nodeLon[node]];
        return {points: pair.map(position), land: position(onLand), connected: !!seen[pair[1]],
          wet: pair.every(node => !!g.waterAt(g.nodeLon[node], g.nodeLat[node]))};
        }""",
        points,
    )
    water_pair = (
    ...
    target = {"lat": chosen["land"][0], "lon": chosen["land"][1]}
    readings = (
        water_leg_readings(wet, "water pair")
        + water_leg_readings(dry, "land point")
        + [
            Reading("both reconstructed points lie on water", chosen["wet"], True),
            Reading("paddle arcs connect them in the journey's direction", chosen["connected"], True),
            Reading("the connected water pair carries no land", w["walked"], 0),
            Reading("every part between the water points is paddled", all(p["kind"] == "paddled" for p in w["legs"][0]["parts"]), True),
...
def a_paddled_profile_is_flat(page: Any) -> Check:
    """Each lake has a read, continuous profile within half a metre of a plane."""
    name = "a paddled profile is flat"
...
def the_walking_modes_never_take_the_water(page: Any) -> Check:
    ...
    for paths in (False, True):
        ...
        readings += water_leg_readings(got, label) + [
            ...
            Reading(
                f"{label}: no water-network source is credited", sum(s["tally"]["sources"].get(k, 0) for k in ("Shore", "Open water", "Streams")), 0
            ),
            Reading(
                f"{label}: neither tap snaps to water, a portage or a launch", not set(got["snappedKinds"]) & {"paddle", "portage", "launch"}, True
            ),
...
THE_PAINT = """(at) => {
  ...
  const read = (name, dx = 0, dy = 0) => { const pane = map.getPane(name);
    ...
    const x = Math.round((mbox.left + pt.x + dx - cbox.left) * (canvas.width / cbox.width));
    const y = Math.round((mbox.top + pt.y + dy - cbox.top) * (canvas.height / cbox.height));
    const d = canvas.getContext('2d').getImageData(x, y, 1, 1).data;
    return [d[0], d[1], d[2], d[3]]; };
  // Noding moved the route's middle sample onto an antialiased stroke edge:
  // alpha 139 rounded two colour channels, while one pixel below was opaque.
  // Compare at a nearby opaque picked pixel with the route under it; choosing
  // the nearest by coverage alone cannot conceal a wrong colour.
  let offset = [0, 0], nearest = Infinity;
  const radius = picked ? Math.ceil(picked.options.weight / 2) : 0;
  for (let dy = -radius; dy <= radius; dy++) for (let dx = -radius; dx <= radius; dx++) {
    const top = read('trailsPicked', dx, dy), under = read('trailsPlanRoute', dx, dy);
    const distance = dx * dx + dy * dy;
    if (top && top[3] === 255 && under && under[3] > 0 && distance < nearest) {
      offset = [dx, dy]; nearest = distance;
    }
  }
  ...
  return {copy: !!picked, clickable: picked ? picked.options.interactive : null,
          ...
          offset, painted: read('trailsPicked', ...offset), onRoute: read('trailsPlanRoute', ...offset)}; }"""
    ...
    **Measured off the canvases and not off a screenshot.** Every pane renders
    ...
    return Check(
        ...
        [
            ...
            Reading("the compared pixel is opaque", bool(drawn["painted"]) and drawn["painted"][3] == 255, True, note=f"offset {drawn['offset']} px"),
...
def the_pinch_waits_for_release(page: Any) -> Check:
    """Drive pinch releases and read replacement opacity and ground in drawn frames.
    ...
    try:
        ...
        probe.evaluate(
            ...
            p.covers = (fine, coarse) => fine.z > coarse.z &&
                Math.floor(fine.x / 2 ** (fine.z - coarse.z)) === coarse.x &&
                Math.floor(fine.y / 2 ** (fine.z - coarse.z)) === coarse.y;
            p.reset = () => {
                p.asked = layers.map(() => ({})); p.early = layers.map(() => ({}));
                p.first = layers.map(() => ({})); p.fades = layers.map(() => null); p.stacks = layers.map(() => []);
            };
            p.observe = () => {
                if (p.watch) layers.forEach((l, i) => {
                    const tiles = Object.values(l._tiles);
                    tiles.filter(t => t.current && t.loaded && t.el.classList.contains('leaflet-tile-loaded')).forEach(t => {
                        const opacity = Number(getComputedStyle(t.el).opacity), key = l._tileCoordsToKey(t.coords);
                        const under = tiles.filter(o => !o.current && o.loaded && p.covers(o.coords, t.coords)).length;
                        if (opacity > 0 && !p.first[i][key]) p.first[i][key] = {opacity, under};
                        if (opacity > 0 && opacity < 1) {
                            if (!p.fades[i]) p.fades[i] = {key, opacity, under};
                            if (under) p.stacks[i].push({key, opacity, under});
                        }
                    });
                });
                p.raf = requestAnimationFrame(p.observe);
            };
            p.observed = async () => {
                await new Promise(requestAnimationFrame); await new Promise(requestAnimationFrame);
                p.watch = false;
                return {first: p.first, fades: p.fades, stacks: p.stacks};
            };
            p.reset(); p.observe();
        ...
        readings.append(Reading("all six layers drawn", names, ["sheet", "shade", "slope", "vegetation", "forest", "mire"]))
        ...
        for target in (16, 12, 15, 12):
            result = probe.evaluate(
                ...
                const replacements = p.layers.map((l, i) => Object.values(l._tiles).filter(t => t.current &&
                    startingTiles[i].some(o => o.loaded && p.covers(o.coords, t.coords)))
                    .map(t => l._tileCoordsToKey(t.coords)));
                const answer = {start, target, left, landed, before, held, during, early: p.early, asked: p.asked, released, replacements};
                p.phase = 'rest'; p.watch = true; p.releaseLoads();
                return answer;
            }""",
                target,
            )
            probe.wait_for_function(clean, timeout=60_000)
            frames = probe.evaluate("() => pinchGroundProbe.observed()")
            ...
            label = f"{result['start']} → {target}"
            for i, name in enumerate(names):
                prefix = f"{label} {name}"
                old = result["released"][i]
                left, landed = result["left"][i], result["landed"][i]
                expected_old = left > landed if i else left < landed or left - landed <= 3
                readings.extend(
                    [
                        ...
                        Reading(f"{prefix}: real images load", settled[i]["images"] > 0, True, note=f"active 256 px images: {settled[i]['images']}"),
                    ]
                )
                if i and left > landed:
                    replacing = [frames["first"][i].get(key) for key in result["replacements"][i]]
                    readings.extend(
                        [
                            Reading(f"{prefix}: replacement tiles were observed", len(replacing) > 0, True),
                            Reading(
                                f"{prefix}: each replacement first shows at full opacity with no held tile beneath",
                                replacing,
                                [{"opacity": 1, "under": 0}] * len(replacing),
                            ),
                            Reading(f"{prefix}: no drawn frame stacks a fading tile over held ground", frames["stacks"][i], []),
                        ]
                    )
                if not i or left < landed:
                    readings.append(
                        Reading(f"{prefix}: a new tile still fades", frames["fades"][i] is not None, True, note=f"frame: {frames['fades'][i]}")
                    )
        ...
        probe.evaluate("""() => {
            ...
            p.reset(); p.watch = true; p.phase = 'drag'; p.dragEnded = false;
        ...
        readings.append(Reading("ring read before drag ends", drag["ended"], False))
        for i, (name, layer) in enumerate(zip(names, drag["layers"], strict=True)):
            readings.extend(
                [
                    Reading(
                        f"drag {name}: asks for the new column while dragging",
                        layer["asked"].get("15", 0) > 0,
                        True,
                        note=f"created per level: {layer['asked']}",
                    ),
                    Reading(
                        f"drag {name}: viewport and one ring active before moveend",
                        layer["filled"],
                        True,
            ...
            fade = frames["fades"][i]
            readings.append(
                Reading(f"drag {name}: a tile fades with no old ground beneath", fade is not None and fade["under"] == 0, True, note=f"frame: {fade}")
            )
        readings.append(Reading("no script errors", errors, []))
    finally:
        probe.close()
    return Check("the pinch waits for release and overlays replace finer ground without fading", readings)
...
def the_sheet_retains_ground(page: Any) -> Check:
    ...
    Returns:
        Retained tile counts while loading and after pruning, and blend modes.
    """
    probe = page.context.browser.new_page(viewport={"width": 390, "height": 844})
    errors: list[str] = []
    ...
    try:
        ...
        probe.evaluate(
            ...
            boxes.forEach(box => { if (!box.checked) box.click(); });
            const names = ['sheet', 'shade', 'slope', 'vegetation', 'forest', 'mire'];
            const name = layer => names.slice(1).find(n => layer.options['trails' + n[0].toUpperCase() + n.slice(1)]) || 'sheet';
            const layers = Object.values(map._layers).filter(layer => layer instanceof L.TileLayer)
                .sort((a, b) => names.indexOf(name(a)) - names.indexOf(name(b)));
            const p = window.retentionProbe = {map, boxes, layers, names: layers.map(name), samples: [], enabled: false};
            p.blended = layers.filter(layer => /trails-(slope|vegetation|forest|mire)-tiles/.test(layer.options.className || ''));
            p.snapshot = event => ({event,
                left: layers.map((layer, i) => Object.values(layer._tiles).filter(t => t.coords.z === p.left[i]).length),
                old: layers.map(layer => Object.values(layer._tiles).filter(t => t.coords.z !== layer._tileZoom).length),
                loading: layers.map(layer => !!layer.isLoading()),
                modes: p.blended.map(layer => getComputedStyle(layer.getContainer()).mixBlendMode)});
            p.record = event => { if (p.enabled) p.samples.push(p.snapshot(event)); };
        }""")
        )
        probe.evaluate("at => { retentionProbe.map.setView(at, 15, {animate: false}); }", SCENE.position)
        clean = """() => retentionProbe.layers.every(layer => !layer.isLoading() && Object.values(layer._tiles).length > 0 &&
            Object.values(layer._tiles).every(t => t.coords.z === layer._tileZoom && t.active))"""
        probe.wait_for_function(clean, timeout=60_000)
        rest = probe.evaluate("""() => {
            const p = retentionProbe;
            p.left = p.layers.map(layer => layer._tileZoom);
            return {...p.snapshot('rest'), names: p.names, boxes: p.boxes.every(box => box.checked)};
        }""")
        readings.extend(
            [
                Reading(
                    "every overlay on, sheet and five overlays drawn",
                    [rest["boxes"], rest["names"]],
                    [True, ["sheet", "shade", "slope", "vegetation", "forest", "mire"]],
                ),
                Reading("all four overlays multiply at rest", rest["modes"], ["multiply"] * 4),
            ]
        )
        probe.evaluate("""() => {
            const p = retentionProbe;
            p.layers.forEach((layer, index) => {
                ...
                layer.on('load', () => { if (p.enabled) p.loads.push(index); p.record('load'); });
            });
            p.map.on('zoomstart', () => { p.startModes = p.snapshot('zoomstart').modes; });
        ...
        for zoom in (12, 15):
            label = "15 → 12" if zoom == 12 else "12 → 15"
            probe.evaluate(
                """z => {
                const p = retentionProbe;
                p.enabled = false; p.ended = false; p.samples = []; p.loads = [];
                p.left = p.layers.map(layer => layer._tileZoom);
                p.map.setZoom(z, {animate: true});
            }""",
                zoom,
            )
            probe.wait_for_function("() => retentionProbe.ended", timeout=10_000)
            probe.wait_for_function(clean, timeout=60_000)
            result = probe.evaluate("""() => {
                const p = retentionProbe; p.record('settled'); p.enabled = false;
                return {samples: p.samples, startModes: p.startModes, loads: [...new Set(p.loads)].length,
            ...
            end, settled = samples[0], samples[-1]
            readings.append(Reading(f"{label}: zoom ends with all six new levels loading", end["loading"], [True] * 6))
            for index, name in enumerate(rest["names"]):
                readings.append(
                    Reading(
                        f"{label}: {name} holds the left zoom while loading"
                        if index == 0 or zoom == 12
                        else f"{label}: {name} drops the left zoom while loading",
                        end["left"][index] > 0 if index == 0 or zoom == 12 else end["left"][index] == 0,
                        True,
                        note=f"left-zoom tiles: {end['left'][index]}",
                    )
                )
            readings.extend(
                [
                    Reading(
                        f"{label}: overlays hold only the left zoom on the way out and none on the way in",
                        all(all(row["old"][i] == (row["left"][i] if zoom == 12 else 0) for i in range(1, 6)) for row in samples),
                        True,
                        note=f"peak per layer: {[max(row['old'][i] for row in samples) for i in range(6)]}",
                    ),
                    Reading(f"{label}: once loaded and active, no layer holds another zoom", settled["old"], [0] * 6),
                    Reading(f"{label}: once loaded and active, no layer holds the left zoom", settled["left"], [0] * 6),
                    Reading(f"{label}: every new level fires load", result["loads"], 6),
                    Reading(
                        f"{label}: every new level contains real loaded images",
                        all(n > 0 for n in result["good"]),
                        True,
                        note=f"current 256 px images per layer: {result['good']}",
                    ),
                    Reading(f"{label}: four overlays multiply at zoomstart", result["startModes"], ["multiply"] * 4),
                    Reading(
                        f"{label}: four overlays multiply through loading, frames and rest",
                        all(row["modes"] == ["multiply"] * 4 for row in samples),
                        True,
                    ),
                ]
            )
        readings.append(Reading("no script errors", errors, []))
        return Check("the sheet retains old ground both ways with every overlay on", readings)
    finally:
        probe.close()
...
def drive(page: Any) -> list[Check]:
    ...
    if wanted(the_kayak_panel_counts_the_whole_way):
        checks.append(timed(the_kayak_panel_counts_the_whole_way, page))
    if wanted(a_kayak_rounds_land_from_open_water):
        checks.append(timed(a_kayak_rounds_land_from_open_water, page))
    if wanted(a_kayak_uses_water_before_land):
        checks.append(timed(a_kayak_uses_water_before_land, page))
    ...
    if wanted(the_walking_modes_never_take_the_water):
        checks.append(timed(the_walking_modes_never_take_the_water, page))
