...
**Old ground across a zoom is the sheet's only** (2026-09-19/20, plan phases 8d–8h). With
the mire on, every overlay on, Safari ended both pages — *"A problem repeatedly occurred"*, the
quiet reload to the initial zoom first — when the finger lifted after zooming in and out several
times in a row; any level, any pan, any length of time, and two tile layers, were fine. Found by
measuring on `forge` and by Uwe's one-minute experiments on the phone, and named only at the
third attempt: not the page (heap and DOM flat), not the tile count (a zoom holds fewer tiles
than a pan, Leaflet drops the left level's unloaded tiles), not the kernel (no `JetsamEvent`),
not the online pack fill (airplane mode dies), not the requests (over unkept, empty ground it
holds), not the vectors, and — after two phases built on it — **not the blend**: with four
multiplied overlays it died where three held, the blend was suspended through the zoom (8d)
and to the pruning of the old level (8e), and `?blend=never` still died. Not Leaflet's Safari
`image-rendering` rule either (`?tiles=plain` died). **`?ground=drop` held**, with a flicker at
every zoom: what Leaflet keeps across a zoom change — the nearest loaded parent on a zoom in,
up to three levels of children on a zoom out, the depth three being phase 2's — is scaled up
to 16× and rasterised by WebKit at the new scale when the finger lifts, a screen's worth of
pixels a level a layer, eighteen screens at once with six layers, freed later than the next
release comes. Settled (8h): the retention override keeps old ground for the sheet only
(`retainGround` on the base tile layer), the overlays let their old level go at the zoom
change and fade in afresh — three screens instead of eighteen, the sheet never blank. The
blend machinery and the measuring switches of 8d–8g are gone; the four overlays multiply at all
times as before. If the fast cycle still ends the page, the sheet's children depth goes back to
Leaflet's two, then one. Two rules this cost: a fix that takes back a decided behaviour is
...
- **2026-09-20** — old ground across a zoom is the sheet's only (§6.13, plan phase 8h): the
  ...
  lifted; the overlays keep none now. The blend machinery of 8d/8e and the switches of 8g are
  gone; the overlays multiply at all times again.
