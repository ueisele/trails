...
class TestTileRange:
    ...
    def test_abisko_at_z13(self):
        assert lantmateriet.tile_range(FIRST_BOX, 13) == (4509, 1932, 4528, 1950)

    def test_abisko_at_z17(self):
        assert lantmateriet.tile_range(FIRST_BOX, 17) == (72144, 30915, 72453, 31201)

    def test_the_world_at_z0(self):
        assert lantmateriet.tile_range((-180, -85, 180, 85), 0) == (0, 0, 1, 0)

    def test_count_over_zooms(self):
        assert lantmateriet.tile_count(FIRST_BOX, [13]) == 380
        assert lantmateriet.tile_count(FIRST_BOX, range(8, 18)) == 118_967
        # Widened 2026-09-13 (decisions §9.24): 27,681 more, and 610 height tiles instead of 540.
        assert lantmateriet.tile_count(ABISKO, [13]) == 440
        assert lantmateriet.tile_count(ABISKO, range(8, 18)) == 146_995
