...
#: What every build here does the same way: the CRS, the register's names and
#: the distances the derived fields are read by, handed to :mod:`graphs`.
RULES = Rules(
...
class Params(graphs.Params):
    """Everything that decides what the Norwegian network comes out as.

    The shared fields are :class:`trails.network.graphs.Params`; these two are
    Norway's own.
...
#: The rendering :func:`graphs.with_capture_date` does, under the name the
#: tests and this module have always used for it.
_with_capture_date = graphs.with_capture_date
...
def fingerprint(sources: list[NetworkSource], masks: Masks, params: Params, protected: gpd.GeoDataFrame) -> str:
    ...
    :func:`graphs.fingerprint` under this country's rules; the text it hashes is
    the one every graph in the cache was keyed by.
