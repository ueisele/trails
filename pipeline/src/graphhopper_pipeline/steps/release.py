...
#: Tags this pipeline creates, e.g. "v2026-08-11-geonorge-20260809". Used to make
#: sure cleanup only ever deletes its own releases.
OWN_TAG_PATTERN = re.compile(r"^v\d{4}-\d{2}-\d{2}-[a-z]+-")
...
class CreateReleaseStep(PipelineStep[ReleaseArtifacts, str]):
    ...
    def execute(self, context: PipelineContext, input_data: ReleaseArtifacts) -> StepResult[str]:
...
"""
    ...
    def _cleanup_old_releases(self, keep_count: int) -> None:
        ...
        # Keep only the tags this pipeline creates, so a hand-made release is
        # never deleted. Matching on the current year would stop cleaning up as
        # soon as the year rolls over, and the country cannot be matched at all
        # yet: _generate_version puts no country code in the tag, so the old
        # `country_code in tag` test only ever passed by accident, "no" being a
        # substring of "geonorge". Extending the tag format is a prerequisite for
        # multi-country support.
        own_releases = [tag for tag in releases if OWN_TAG_PATTERN.match(tag)]

        # Delete old releases
        if len(own_releases) > keep_count:
            for tag in own_releases[keep_count:]:
